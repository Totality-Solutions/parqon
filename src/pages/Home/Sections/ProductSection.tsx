  ...
  return (
    <Section id="products" className="py-0 2xl:py-0 !py-0 transition-all">
      ...
      >
        ...
        <button className="hidden lg:block text-parqon-brown transition-transform duration-300 hover:translate-x-2">
          <img src={iconSrc} alt="" className="w-10 h-10 2xl:w-16 2xl:h-10 object-contain" />
        </button>
      </motion.div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 2xl:gap-24">
        ...
        <div className="lg:hidden relative z-20">
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="w-full flex justify-between items-center px-6 py-5 border border-gray-200 bg-white"
          >
            <span className="text-dynamic-body font-bold tracking-widest text-parqon-brown">{activeTab}</span>
        ...
        <motion.div 
          ...
          className="hidden lg:flex lg:w-1/4 flex-col overflow-visible"
        >
          {(Object.keys(productData) as Category[]).map((tab) => (
            <button
              key={tab}
              onClick={() => handleTabChange(tab)}
              className={`group flex items-center gap-5 w-full text-left px-8 py-6 transition-all duration-500 border-b border-gray-50 ${activeTab === tab ? 'bg-gray-50/50 text-parqon-brown border-parqon-brown' : 'text-gray-400 hover:text-gray-900 border-transparent'}`}
            >
              <div className={`w-5 h-5 rounded-full border-2 items-center justify-center flex transition-all duration-500 ${activeTab === tab ? 'border-parqon-brown bg-white' : 'border-gray-200 group-hover:border-gray-400'}`}>
                {activeTab === tab && <div className="w-2 h-2 rounded-full bg-parqon-brown" />}
              </div>
        ...
        <div className="w-full lg:w-3/4">
          <motion.div
            ...
            className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 2xl:gap-10"
          >
            {productData[activeTab].images.map((img, idx) => (
              <motion.div
                key={`${activeTab}-${idx}`}
