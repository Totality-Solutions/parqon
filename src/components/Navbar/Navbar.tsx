...
export const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Adjusted to 50px for a more stable trigger on high-end monitors
  ...
  return (
    // Increased z-index to 999 to ensure it stays above Project Hero visuals
    <nav className={`bg-white sticky top-0 z-[999] border-b border-gray-50 transition-all duration-500 ease-in-out ${
      isScrolled ? 'py-3 shadow-sm' : 'py-4 md:py-8'
    }`}>
      <Container className="flex flex-col items-center">
        ...
        <div className="w-full relative flex items-center justify-center">
          {/* Mobile Scrolled Header Header Overlay */}
          {isScrolled && (
            <div className="md:hidden w-full flex justify-between items-center relative z-[120]">
               <span className="text-[10px] font-bold tracking-widest text-gray-400 uppercase">Paroon</span>
               <button onClick={toggleMobileMenu}>
                  ...
                  <AnimatePresence>
                    {isProducts && showSubmenu && (
                      <motion.div
                        ...
                        exit={{ opacity: 0, y: -5 }}
                        transition={{ duration: 0.2 }}
                        className="fixed left-0 w-screen pointer-events-none"
                        style={{ top: isScrolled ? '60px' : '160px' }}
                      >
                        <div className="pointer-events-auto relative bg-white border-t border-gray-50 shadow-xl">
                          <div className="absolute top-[-40px] left-0 w-full h-[40px] bg-transparent" />
                          <ProductSubmenu onClose={() => setShowSubmenu(false)} />
                        </div>
                      </motion.div>
